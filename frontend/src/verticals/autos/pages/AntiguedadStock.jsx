...
const RANGE_BG = {
  green: 'bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-400 border-green-200',
  yellow: 'bg-yellow-100 dark:bg-yellow-900 text-yellow-700 dark:text-yellow-400 border-yellow-200',
  orange: 'bg-orange-100 text-orange-700 border-orange-200',
  red: 'bg-red-100 dark:bg-red-900 text-red-700 dark:text-red-400 border-red-200',
}
...
export default function AntiguedadStock() {
  ...
  return (
    <div className="space-y-6 pb-20 lg:pb-0">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-3">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400">Análisis de envejecimiento del inventario</p>
          </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="card">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm text-gray-500 dark:text-gray-400">Total Unidades</span>
          </div>
          <p className="text-2xl font-bold text-gray-900 dark:text-white">{resumen.total_unidades || 0}</p>
        </div>
        <div className="card">
          <div className="flex items-center gap-2 mb-2">
            <Clock className="w-5 h-5 text-yellow-600 dark:text-yellow-400" />
            <span className="text-sm text-gray-500 dark:text-gray-400">Promedio Días</span>
          </div>
          <p className="text-2xl font-bold text-gray-900 dark:text-white">{resumen.promedio_dias || 0}</p>
          <p className="text-xs text-gray-400">Mediana: {resumen.mediana_dias || 0}</p>
        </div>
        <div className="card">
          <div className="flex items-center gap-2 mb-2">
            <DollarSign className="w-5 h-5 text-green-600 dark:text-green-400" />
            <span className="text-sm text-gray-500 dark:text-gray-400">Inversión Total</span>
          </div>
          <p className="text-2xl font-bold text-green-600 dark:text-green-400">{formatCurrency(resumen.inversion_total)}</p>
        </div>
        <div className="card">
          <div className="flex items-center gap-2 mb-2">
            <AlertTriangle className="w-5 h-5 text-red-600 dark:text-red-400" />
            <span className="text-sm text-gray-500 dark:text-gray-400">Más Antiguo</span>
          </div>
          {resumen.unidad_mas_antigua ? (
            <>
              <p className="text-lg font-bold text-red-600 dark:text-red-400">{resumen.unidad_mas_antigua.dias_en_stock}d</p>
      ...
      {/* Gráfico */}
      <div className="card">
        <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Distribución por Antigüedad</h3>
          ...
          <div key={rango.key} className="card">
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <span className="text-lg font-bold text-gray-900 dark:text-white">{rango.cantidad}</span>
                ...
                <table className="w-full text-sm">
                  <thead className="bg-gray-50 dark:bg-gray-800">
                    <tr>
                      ...
                      <th className="px-3 py-2 text-left font-medium text-gray-500 dark:text-gray-400">Dominio</th>
                      <th className="px-3 py-2 text-right font-medium text-gray-500 dark:text-gray-400">Días</th>
                      <th className="px-3 py-2 text-right font-medium text-gray-500 dark:text-gray-400">Costo</th>
                      <th className="px-3 py-2 text-right font-medium text-gray-500 dark:text-gray-400">Publicado</th>
                  ...
                  <tbody className="divide-y">
                    {rango.unidades.map(u => (
                      ...
                      >
                        ...
                        <td className="px-3 py-2 text-gray-600 dark:text-gray-400">{u.dominio}</td>
                        <td className={clsx(
                          'px-3 py-2 text-right font-semibold',
                          u.dias_en_stock > 60 ? 'text-red-600 dark:text-red-400' : u.dias_en_stock > 30 ? 'text-yellow-600 dark:text-yellow-400' : 'text-green-600 dark:text-green-400'
                        )}>
                          {u.dias_en_stock}d
                        </td>
                        <td className="px-3 py-2 text-right text-gray-600 dark:text-gray-400">{formatCurrency(u.costo_total)}</td>
                        <td className="px-3 py-2 text-right">{u.precio_publicado ? formatCurrency(u.precio_publicado) : '-'}</td>
                        <td className="px-3 py-2">
                          <span className={clsx(
                            'px-2 py-0.5 rounded text-xs font-medium',
                            u.estado === 'disponible' ? 'bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-400' :
                            u.estado === 'reservado' ? 'bg-yellow-100 dark:bg-yellow-900 text-yellow-700 dark:text-yellow-400' :
